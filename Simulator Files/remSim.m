...
if handles.Sim.rFromFile == 1
    ...
    if ~eflag
        handles.Network.r = ones(size(handles.Train.trials, 1)) .* handles.Network.rParam;
...
handles.simLegend = legend(handles.Data.stage(handles.Sim.currentStage).patNames, 'Position', lPos);
legend('boxoff');
set(get(handles.simAxes, 'XLabel'), 'String', handles.Sim.XLab);
set(get(handles.simAxes, 'YLabel'), 'String', handles.Sim.YLab);
